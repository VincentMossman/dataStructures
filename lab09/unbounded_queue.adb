...
package body Unbounded_Queue is
   ...
   begin
      if Queue.Front = null then    -- Is the queue empty?
         ...
         Queue.Front := new Node_Type'(Position => 1, Info => Item,  Next => null);
         Queue.Rear  := Queue.Front;
      else
         -- No, link a new node to the rear of the existing queue.
         Queue.Rear.all.Next := new Node_Type'(Position => Queue.Rear.all.Position + 1, Info => Item,  Next => null);
